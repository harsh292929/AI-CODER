import os
from ...api import call_harsh_api_with_pagination, extract_and_parse_xml
...
def find_file_with_harsh(filename, project_context, max_retries=2, current_retry=0):
    if os.path.exists(filename):
        return filename
...
"""
    ...
    try:
        ...
        if suggested_file:
            ...
            return find_file_with_harsh(suggested_file, project_context, max_retries, current_retry + 1)
        else:
            print_error("harsh couldn't suggest an alternative file.")
            return None
    except Exception as e:
