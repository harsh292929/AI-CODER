from src.drd.cli.query.file_operations import get_files_to_modify, get_file_content, find_file_with_harsh
import unittest
...
class TestFileOperations(unittest.TestCase):
    ...
    @patch('src.drd.cli.query.file_operations.call_harsh_api')
    def test_get_files_to_modify(self, mock_call_harsh_api):
        mock_call_harsh_api.return_value = """
        <response>
          <files>
    ...
    @patch('os.path.exists')
    @patch('src.drd.cli.query.file_operations.call_harsh_api')
    def test_find_file_with_harsh_existing_file(self, mock_call_harsh_api, mock_exists):
        mock_exists.return_value = True
        result = find_file_with_harsh(
            'existing_file.txt', 'Test project context')
        self.assertEqual(result, 'existing_file.txt')

    @patch('os.path.exists')
    @patch('src.drd.cli.query.file_operations.call_harsh_api')
    def test_find_file_with_harsh_non_existing_file(self, mock_call_harsh_api, mock_exists):
    ...
    @patch('os.path.exists')
    @patch('src.drd.cli.query.file_operations.call_harsh_api')
    def test_find_file_with_harsh_max_retries(self, mock_call_harsh_api, mock_exists):
        mock_exists.return_value = False
        mock_call_harsh_api.return_value = """
        <response>
          <file></file>
