from ..utils.api_utils import call_harsh_api_with_pagination, call_harsh_vision_api_with_pagination, stream_claude_response, parse_paginated_response
import xml.etree.ElementTree as ET


def stream_harsh_api(query, include_context=False, instruction_prompt=None):
    xml_buffer = ""
...
def call_harsh_api(query, include_context=False, instruction_prompt=None):
    response = call_harsh_api_with_pagination(
        query, include_context, instruction_prompt)
    return parse_paginated_response(response)


def call_harsh_vision_api(query, image_path, include_context=False, instruction_prompt=None):
    response = call_harsh_vision_api_with_pagination(
