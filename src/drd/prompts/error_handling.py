import traceback
import click
from ..utils.api_utils import call_harsh_api_with_pagination
from ..api.harsh_parser import parse_harsh_response, extract_and_parse_xml, pretty_print_commands
from ..utils import print_error, print_success, print_info
from ..metadata.common_utils import generate_file_description
import xml.etree.ElementTree as ET


def handle_error_with_harsh(error, cmd, executor, metadata_manager, depth=0, previous_context=""):
    if depth > 3:
        print_error(
...
# Instructions for harsh: Error Resolution Assistant
...
"""
    ...
    print_info("Sending error information to harsh for analysis...")
    response = call_harsh_api_with_pagination(
        error_query, include_context=True)

    try:
        # Use the existing extract_and_parse_xml function for better error handling
        root = extract_and_parse_xml(response)
        # Use the existing parse_harsh_response function
        fix_commands = parse_harsh_response(response)
    except ValueError as e:
        print_error(f"Error parsing harsh's response: {str(e)}")
        return False

    print_info("harsh's suggested fix:")
    pretty_print_commands(fix_commands)
    print_info("Applying harsh's suggested fix...")
    ...
    if fix_applied:
        ...
        return handle_error_with_harsh(Exception(error_message),
                                        {"type": "fix",
