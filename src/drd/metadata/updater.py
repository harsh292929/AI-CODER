...
from ..api.harsh_parser import extract_and_parse_xml
...
def find_file_with_harsh(filename, project_context, folder_structure, max_retries=2, current_retry=0):
...
"""
    ...
    response = call_harsh_api_with_pagination(query, include_context=True)
    ...
    try:
        ...
        if suggested_file and suggested_file.strip():
            print_info(
                f"harsh suggested an alternative file: {suggested_file}")
            return find_file_with_harsh(suggested_file.strip(), project_context, folder_structure, max_retries, current_retry + 1)
        else:
            print_info("harsh couldn't suggest an alternative file.")
            return None
...
def update_metadata_with_harsh(meta_description, current_dir):
...
"""
    ...
    files_response = call_harsh_api_with_pagination(
    ...
    try:
        ...
        for file_info in files_to_process:
            ...
            found_filename = find_file_with_harsh(
